...
export default function ThemeToggle({ className = '' }: ThemeToggleProps) {
  ...
  return (
    ...
    >
      {isDark ? (
        <Sun className="w-4 h-4 animate-icon-swap transition-transform duration-200 group-hover:rotate-12" aria-hidden="true" />
      ) : (
        <Moon className="w-4 h-4 animate-icon-swap transition-transform duration-200 group-hover:-rotate-12" aria-hidden="true" />
      )}
